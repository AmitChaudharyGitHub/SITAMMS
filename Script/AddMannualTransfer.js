...
$(document).ready(function () {
    $("#Project").change(function () {
        ...
        if ($("#Project option:selected").val() !== 0) {
            ...
            $.ajax({
                ...
                error: function (ex) {
                    alert('Failed to retrieve Mannual Transfer No.' + ex);
                }
            });
            return false;
        }
    ...
    $("#PurchaseIRDate").change(function () {
        $("#PurchaseIRNo").empty();
        if ($("#Project").val() == '0' || $("#ddlPurchaseType").val() == '') {
            alert('Transfer Type and Project Field Require!');
            $("#PurchaseIRDate").val('');
            return;
        }
        if ($("#Project option:selected").val() !== '') {
            ...
            $.ajax({
                type: 'POST',
                url: GetPurchaseIRNodateWise,

                dataType: 'json',

                data: { ProjectID: $("#Project").val(), OutPIType: $("#ddlPurchaseType").val(), ManPurDate: $("#PurchaseIRDate").val() },
                complete: function () {
                    $('#dvLoading').hide();
                },
                success: function (obj) {
...
$(document).ready(function () {
    ...
    $("#addNew").click(function () {
        ...
        if (gridList.findIndex(s=>s.ItemNo == obj.ItemNo) > -1) {
...
function Moveto() {
    var movstr = $("#PurchaseIRNo").val().replace("/", "#");
    //debugger;
    //var r = confirm("If you want to add tax then click OK else click Cancel.?? ");
    //if (r == true) {
        window.location.href = "/IndentPurchaseOrders/CreateMannualPurchaseOrder?" + $.param({ PI: movstr });
    //} else {

    //    window.location.href = RedUrlToIndex;
    //}
    //debugger;
}