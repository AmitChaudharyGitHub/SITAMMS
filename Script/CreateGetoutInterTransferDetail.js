$(document).ready(function () {
    ...
    debugger;
    $.getJSON(GetInterProjectDetail, { TransferNo: $("#hdnTransferNo").val() }, function (data) {
        data = $.parseJSON(data);
        debugger;
        $.each(data, function (i, item) {
            ...
            $('#lblGRNNo').text(item.GRNumber);
            $('#hdn_InterProjectId').val(item.ProjectId);
            $('#hdn_TransferProjectId').val(item.TRansferProjectID);
            $('#hdn_InterTransferMasterId').val(item.InterTransferMasterTransId);
            $('#txtInterTransferNo').val($('#hdnTransferNo').val());
            $('#lastdateOfreceive').text(item.ReachingDateofDestination);
        ...
        BindGrid($('#hdnTransferNo').val(), $('#hdn_InterProjectId').val());
 ...
 $("#Submit").click(function (e) {
     ...
     if ($('#txtGetoutDate').val()=='') {
         alert('Please enter Gate Out Date');
         return false;
     }
     ...
     if (valid) {
         alert($("#hdnTransferType").val());
         var json = JSON.parse(gridTojson());
         console.log(gridTojson());
         console.log(json);
         ...
         if ($(this)[0].rowIndex != 0) {
             ...
             x.each(function () {
                 if ($(this).children('input').length > 0) {
                     ...
                     var x = $(this).text();
                    // alert(x.replace(/"/g, "").replace('"', '_'));
                    // itArr.push('"' + x.replace('"', "").replace('"', '_') + '"');
                     itArr.push('"' + x.replace(/"/g, "").replace('"', '_').trim() + '"');
                 }
             });

             itArr.push('"' + $("#txtGetOutNo").val() + '"');
             itArr.push('"' + $("#hdn_InterProjectId").val() + '"');
             itArr.push('"' + $("#hdn_TransferProjectId").val() + '"');
             itArr.push('"' + $("#txtInterTransferNo").val() + '"');
             ...
             itArr.push('"' + $("#hdn_InterTransferMasterId").val() + '"'); 
 ...
 $("#txtGetoutDate").change(function () {
     CreateGetoutNo($('#hdn_InterProjectId').val(), $('#hdnTransferType').val(), $("#txtGetoutDate").val());
 });
